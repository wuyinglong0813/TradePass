...
> 本文是未来演进草案，不是当前部署说明。当前仓库是单个 Spring Boot 应用，使用 MySQL 持久化业务数据，并使用 Redis 提供缓存和限流；准确现状见 `docs/backend-architecture.md`。
...
- 随业务量和可靠性需求再评估 Sentinel、RocketMQ 与 Redis Sentinel/Cluster，避免提前引入未被使用的基础设施。
